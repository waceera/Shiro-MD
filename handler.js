    ...
    try {
        ...
        try {
            ...
            if (user) {
                ...
                if (!("ask" in user)) user.ask = [];
                if (!("acces" in user)) user.acces = [];
                if (!("partRes" in user)) user.partRes = false
                if (!user.registered) {
                    if (!("name" in user)) user.name = m.name
                    if (!isNumber(user.aiselect)) user.aiselect = 0
                    if (!isNumber(user.acces)) user.acces = 0
...
`.trim(), data.jid)
        ...
        typeof plugin.command === 'string' ?
            ...
            false;
             if (!isAccept) continue; // Jika command tidak diterima, lanjutkan
                m.plugin = name
                ...
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    this.reply(m.chat, `Limit harian kamu telah habis\n\nGunakan perintah *!Claim* atau jika kamu ingin limit tak terbatas/unlimited gunakan perintah *!Premium*`, m)
                    continue
                }
                ...
                try {
                    ...
                    if (e) {
                        ...
                        for (let key of Object.values(global.APIKeys))
                            text = text.replace(new RegExp(key, 'g'), '#Veikonee#')
                        if (e.name)
                            for (let [jid] of global.owner.filter(([number, _, isDeveloper]) => isDeveloper && number)) {
                                let data = (await conn.onWhatsApp(jid))[0] || {}
                                if (data.exists)
    ...
    switch (action) {
        ...
        case 'remove':
            if (chat.welcome) {
                ...
                let gpname = groupMetadata.subject;
                for (let user of participants) {
                    ...
                    let messageTemplate = action === 'add'
                        ? (chat.sWelcome || this.welcome || conn.welcome || `Welcome to @subject, ${username}!`)
                        : (chat.sBye || this.bye || conn.bye || `Goodbye, ${username} 👋`);
                    text = messageTemplate.replace('@subject', gpname);
                    this.sendMessage(id, {
                        text: text,
                        mentions: [user],
                        contextInfo: {
...
let msg = {
    ...
    botAdmin: 'Jadikan bot sebagai *Admin* untuk mengakses fitur ini',
    admin: 'Perintah ini hanya dapat digunakan oleh *Admin Group*',
    restrict: 'Fitur ini *Dibatasi*',
    game: 'Fitur *Game* tidak diaktifkan di chat ini',
    rpg: 'Fitur *Rpg* tidak diaktifkan di chat ini',
    rowner: 'Fitur hanya dapat diakses oleh *Owner*',
